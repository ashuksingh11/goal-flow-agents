...
`goal_state_get` · `goal_accepted` · `speech` (v11).
...
**The voice (v11).** `speech { utterance_id, cue, text, url }` follows the frame it speaks
for — today only `understanding` — and carries a **path, not audio**: synthesis happens when a
UI fetches `GET /speech/<id>.mp3`, the hub's only HTTP route. So the gate never waits on the TTS
provider, and an utterance nobody plays is never billed. The sentence is composed
**deterministically** by the cloud (`_understanding_speech`) rather than by a model: it sits at
the end of the interpretation window v8 spent a milestone shortening, and the material is
already structured. The id is deterministic per (goal, cue) so the replay cache re-sends the
same audio instead of paying twice; an id the cloud never minted is a 404, which is what stops
the route being a synthesis oracle against the account.

**Speech is the one thing here that fails silently, deliberately.** No `FISH_API_KEY` ⇒ the
frame is never sent ⇒ every surface renders exactly as it did in v10. That inverts the LLM-only
rule (§3), and the reason is that the gate behind the voice is already complete and answerable:
an interpretation that cannot run must not pretend, but a voice-over that cannot run costs the
user nothing. The other half of the problem is **autoplay**, which is not ours to fix — a
browser rejects `audio.play()` without user activation, a cross-origin webview additionally
needs `allow="autoplay"` (the Bixby surrogate sets it; a real Hub's native WebView we cannot
touch), and the create phase hands us no gesture, because the webview is mounted by the cloud
while the user is looking at Bixby. So the chat UI never assumes it may speak: it tries, and on
refusal shows a **"Hear this"** tap, which also unlocks every later utterance in that document.
...
429 waits seconds, everything else still waits 400ms, and the cool-off is a floor)**,
**gate 34 (a goal ends when its window AND its plan have both passed — `verify/v11`)**. Gate 18
is RETIRED — it
...
reasons in the device's day**, **31 the voice says the right thing and its absence costs
nothing**, **33 a dispatch is always answered — no exception can hang the UI**. Plus
`scripts/e2e_two_goals.py`, which is NOT a gate: it drives the real two-goal demo headlessly
against a live stack, and it is the only thing that can answer "does a card retire on the
right day" — a question that only exists where the cloud, the device and the clock meet. Everything but 10 and 12 needs no API key; gate 28's interpreter
...
| **v9** | **A UI audit, and the record of the run made to survive it.** Contrast first: `--ink-faint` was **2.74:1** on white while carrying every eyebrow, count, caption and note in the panel — a fail at AA and AA-large, on a door read from 1-2 m — so it becomes an alias of `--ink-quiet` (4.51:1) and all ~48 call sites are fixed at once; `--stroke-strong` arrives for control boundaries, because `--stroke` is 1.23:1 and the Decline button drawn with it read as *disabled*. 26 distinct font sizes (0.75/0.76/0.78, 0.92/0.95 — pairs nobody can tell apart but must still choose between) become **seven fluid steps**. Emoji and bare text glyphs become an inline `Icon` set, so a family mark can take its family's colour and a chevron can *turn* rather than being swapped for `⌃` (U+2303, the Mac control key). **A constraint and a preference stop looking like the same object** — the one thing the confirm gate exists to teach — a tinted box with a disc and a lock against a bare row in a dense list, and the constraint grid drops to one column because an odd count rendered as a half-width orphan. **One live region, not five**: spinner, progress bar, elapsed counter, goal-bar clock and engines-cleared hairline all described a single fact, and the run has no honest percent or ETA, so only the spinner survives and the duration lands once, settled, beside the plan it produced. The refusal gets a **Close now** button and a countdown that is real (`closes_in_s` off the wire, linear, absent when the cloud scheduled no close). Then, from a live run: the goal bar and the heading were printing the same sentence twice, because the understanding frame overwrote what the user said with the cloud's restatement of it. Finally, two subtractions and an addition. **Provenance captions are gone** — every constraint said *"Account · always enforced"* and every preference *"Account · relevance"*: a caption identical on every row is furniture, and *relevance* is the internal name of our own selection pass. The data stays on the wire; it was the display that was noise. **The transcript now outlives the run** (§7), and **it answers for the whole run** — it rendered only the blocks the transcript produced, which is Grounding and nothing else, so four of the seven engines left no trace; every engine that fires now has a section carrying its prose and steps or its own note and verdict. **Task Manager moved to third in the rail**, because the device breaks the goal into tasks before it reads a single world fact and the beat was being fired sixty lines later where the ledger is written — which also filed the whole task breakdown under Grounding, the engine that had not started. **A text pass** took out what the screens were saying twice or saying in our own vocabulary: the chips show the store's authored labels ("peanut allergy") instead of enforcement tokens (`rohan_low_sodium`); the plan card's "what it knew" strip and the board's considered / impact / Actions tail are gone; counts nobody can check ("10 options considered · 5 rejected") and one line of plumbing (`ShoppingList.PlaceOrder · estimatedTotal: 58.2`) with them. **Plan rows gained a mark** — an unmatched row takes the plan's dominant one, because a food-word list grows forever and is still wrong for the next domain. A grounding step's headline is now the FIRST CLAUSE of the tool's `[Description]` rather than all of it: those strings are written for a model ("Call once; do not follow it with GetRecipe") and the transcript was reading a prompt back at the person who asked about dinner. **Four things the screens were saying in our own voice** went with them: the meal feed's `steer` was doubling as the adaptation card's headline, so the fridge asked a family to "say both reasons in the why" (the feed grew an `action` beside each `steer`); the compose schema's `rejected` example — `pork belly stir-fry / no pork` — was being copied verbatim into a HOME-PREP plan, so the placeholders are neutral and the shape says it is all placeholder; the board card's ✓ line was the device's status note ("Executed 3 proposal(s). Goal 100% (7/7 steps done).") and its ➡ line kept an adaptation the user had already approved; and the cross-goal steer's "use up what would spoil before leaving" was being followed by *retitling the dinner* "Use up leftovers before away" — an adapted row is still the same kind of thing it was, and the reason belongs in `why`. And on the device, **a 429 is not a dropped socket**: every retry site waited 400ms x attempt, so three grounding attempts against a quota window measured in seconds were spent inside ~1.2s and produced three raw-exception notices instead of a plan. Rate limits back off in seconds (2s, 4s… capped, jittered), `Retry-After` wins over the guess, everything else keeps its 400ms, and one process-wide cool-off — a floor, never a rollback — means a throttled run slows down once rather than at grounding *and* compose. Gate 30. |
| **v11** | **The fridge speaks.** The understanding gate is read out loud through fish.audio: a new `speech` frame carrying a URL, a single `GET /speech/<id>.mp3` on the hub, and a sentence composed in code rather than by a model — it lands at the end of the interpretation window and there was no interpretation left to do, only assembly. Two decisions carry the feature. **It fails silently**, alone in this system: no key means no frame, and every surface renders exactly as it did in v10, because a gate that is already complete and answerable loses nothing to a missing voice. And **it never assumes it may speak** — autoplay is refused without user activation, the create phase supplies no gesture (the webview is mounted by the cloud while the user is looking at Bixby), and a real Hub's WebView is a native app we cannot configure — so a refusal surfaces as a "Hear this" tap rather than as silence, and the surrogate's iframe carries `allow="autoplay"` so the dev demo is unconditionally in the best case. Synthesis is deferred to the fetch, so nothing about the gate got slower and an unplayed utterance is never billed. Gate 31. |
| **v11.1** | **The voice reaches five screens, and the shape came from a stopwatch.** Timed on a real run: Pre-Check resolves in **0.02s**, Capability Manager in **0.00s**, Task Manager in 2.33s, grounding in 10s+ — so **five of the seven harness engines finish in under 100ms while a spoken sentence takes 2-5 seconds**, and per-engine narration is not a rejected design but an arithmetically impossible one (the voice would announce Pre-Check somewhere around the Planner). The same maths rules out the thinking stream, 22 events in ~15s. What survives is the rule every composer obeys: **the voice is not a second copy of the screen** — the screen is parallel and skimmable, the voice is serial and slow, and its job is to say when you are needed and what changed while you were not looking. So: the gate, two beats while composing (fired on the device's own harness events, never a timer — a timed beat can describe a phase that has ended), the plan, the approvals, and a closure line. **The gate got shorter and the measurement says where the time went**: v11.0 ran 15.0s, naming all four rules is 12.0s, naming only the safety-critical ones is 10.6s, a bare count 8.2s — the NAMES are the cost, not the date window that was cut first, so the allergy stays audible and "no pork" becomes a count. **The plan cue is the one an LLM writes**, as a `narration` field on the device's existing compose call: zero extra round trips at the exact moment the user is waiting for good news, and it cannot contradict the plan because the same model wrote both. There is deliberately **no deterministic fallback** — code can only count rows, and "seven items, three needing approval" describes a data structure rather than a week of dinners. **Emotion tags** (fish.audio `[bracket]` cues, verified as genuinely different audio) are per-cue because emotion is earned by the moment: `[excited]` belongs on the plan and would read on the confirmation gate — over a peanut allergy — as a system that does not grasp what it is holding. Tags never reach the caption. Five cues on one serial channel is a **scheduling** problem, so the chat UI gains a queue: one at a time, the plan cuts off progress chatter, queued progress is dropped rather than deferred, a question is never talked over, any tap stops the voice. **Three bugs were caught before the demo could see them.** `approvals` outranked `plan` and truncated it every time, because the two are sequential on one screen rather than competing. The queue drained on `play()` (which resolves in ~150ms) instead of on the audio ending (4-10s) — the two-voices bug, inside the thing built to prevent it. And `state.speech` was a single slot: React batches, so `plan` and `approvals` arriving in one tick produced one render and the effect saw only the second, silently never speaking the plan summary — **invisible to every headless test, because a headless client has no reducer to batch**, and proven both ways against a fake hub. Also: `SPEECH_ENABLED=false` silences the voice while leaving the key in place (v11.0 argued against a second switch; that argument was about misconfiguration and this is development, and the original worry turns out to be a logging problem — the startup line now names which silence you are in). The `saved` line is 3.1s because the webview unmounts at 3800ms and the first version measured 4.6s, which does not read as a timing bug but as a crash. Gates 31 (cloud, five cues) and 32 (chat, the queue policy). |
| **v11.2** | **Three bugs the demo could not survive, and one doc that caused a fourth.** A provider 429 during grounding made the device throw; `Program.cs` caught it, logged `frame handling failed for dispatch`, and **told nobody** — so the cloud's graph waited in `collect_plan` for a `plan_ready` that never came, the webview sat on "grounding" forever, and the bracket never closed. **Nothing in that chain has a timeout**, so the goal hung until a page reload, and ANY exception during planning did it — the 429 was only the common trigger. `RunAsync` now always answers, with a precheck HOLD the cloud turns into a readable notice and a timed close (an empty plan card would have been a confident lie rather than a stall). Measured against the live provider, because the numbers chose the fix: **~6 requests of ~3.5k tokens saturate Cerebras, no `Retry-After` is returned, and it recovers in ~3 seconds** — so rate limits get their own retry counter instead of consuming one of three compose attempts and appending a "your JSON was unparseable" instruction that GREW the prompt against a tokens-per-window limit. **A goal now ends when its window AND its plan have both passed**, which took three attempts: `Day` is a planner-chosen index (four identical two-day runs returned [1,2,4], [1,5], [1,3], [1,2]) so trusting it left cards on the board past their dates, while trusting the window alone retired a 7-day meal plan under a 4-day window — a goal vanishing with its work visibly unfinished, and the worse failure of the two. Each wrong version passed reasoning and failed a real run. **The `thought` LLM call is deleted**: it ran TWICE per goal (LangGraph re-executes a node on resume from `interrupt()`) for a string no surface has rendered since v9, at `max_tokens=60` on a reasoning model where it mostly came back `finish_reason: length` and fell back anyway — two round trips inside the interpretation window, and two more chances to draw the 429. **On the voice**: a browser handed a chunked mp3 with no `Content-Length` waits for the COMPLETE body, so a 160-character utterance meant 6.1s of silence and the plan/approvals cues were still synthesizing when the webview closed — a cue is now one frame per SENTENCE, warmed in parallel, first audio **6.6s → 1.0s**. The gate stopped asking aloud (nothing is listening), the approvals cue names only what needs a human and never the estimated amount, and the saving dwell grew so the goodbye finishes. Gates 31–34; `scripts/e2e_two_goals.py` drives the whole two-goal demo headlessly and is what found the completion bug. |
